...
- Documentazione: `docs/` — `getting-started.md`, `writing-content.md`, `architecture.md`, `deployment.md`.
- Script: `scripts/validate_posts.rb` (gate PR/deploy), `scripts/sync_youtube.rb`.
- Workflow CI: `.github/workflows/` — `jekyll.yml` (deploy), `checks.yml` (validazione PR), `uptime.yml` (probe).
