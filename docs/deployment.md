...
- **Every `expect_body` is content, so changing content can turn the monitor red on a site that is perfectly up.** That is what it is for — but it means a copy change is a two-file change. It has already happened once: rewriting the home page `<title>` for SEO left `checkfleet.yml` asserting `<title>Home | Allan Nava</title>`, and both the smoke job and the 10-minute uptime monitor went red for hours reporting 12 OK / 1 BAD while every URL answered 200. The home assertion is now the stable prefix `<title>Allan Nava — ` (the tagline after it is marketing copy and must not be able to declare an outage); the rest are full titles, which are stable because they are `"{page.title} | {site.title}"`.
...
**Locations.** Coordinates come from `recordingDetails.location` on the video, and `/map` plots every post that has `lat`/`lng`. Because the sync skips any video already embedded in a post, a place tagged on YouTube *after* the post was created would never reach the map — the post keeps no coordinates forever. `scripts/resync_locations.rb` closes that gap: it runs as a step of this workflow right after the sync, walks the posts that embed a video and have no `lat`/`lng`, asks the API in batches of 50 ids (1 quota unit per batch, so the whole channel costs 3 units of the daily 10,000) and writes the coordinates into the front matter after `author:`. `DRY_RUN=1` lists what it would change; `REFRESH=1` also revisits posts that already have coordinates and updates them when the place changed on YouTube — off by default, because some coordinates were set by hand. Locations that come back as `(0, 0)` are ignored, and videos that answer nothing (private, removed) are reported, not treated as errors.

When the API has no location, the script that creates posts falls back to geocoding a city named in the video description through Nominatim. That fallback needs an **explicit place marker** — 📍, 📸, or "filmed/shot/recorded/located in|at" — because the earlier pattern also accepted bare `in`/`at` with a case-insensitive flag and matched any capitalised word in the description: "Follow me at Instagram" gave `Instagram for more`, "Training in FEBRUARY 2024" gave `ing in FEBRUARY`, "notifications" gave `ions`. Nominatim answers *something* for all of those, and the marker lands on another continent.
...
It installs `webp` and `ffmpeg`; Chrome comes with the runner image. The commit step runs with `if: always()`, so if one generator fails the other one's work is still committed — the first version threw away 199 freshly encoded variants because a later step died.

**It used to install `imagemagick` too, and that package never did anything** (#141). `generate_og_cards.rb` looked for a binary called `magick`, which is ImageMagick **7**; Ubuntu's `imagemagick` package is version **6** and installs `convert`. So the card step aborted with *"Serve sips o ImageMagick"* on a runner where ImageMagick was installed, and the workflow failed three scheduled runs in a row — which is also why the four newest project posts had no card of their own. Both generators now try `sips` → `magick` → `convert` → `ffmpeg` and say which ones they looked for when none is found; in CI the converter is **ffmpeg**, already there for AVIF, so no package exists just to make JPEGs. `JPEG_TOOL=convert ruby scripts/generate_og_cards.rb` forces one branch, which is how you exercise the CI path from a Mac (it will tell you the binary is missing rather than silently picking `sips`).
...
Due categorie di finding vanno lette diversamente. I **403** sono spesso falsi positivi: siti vivi che bloccano i bot — quelli verificati vivi stanno in `--ignore-urls` (vedi [Getting started](getting-started.md)). I **404 su `i.ytimg.com/vi/<id>/hqdefault.jpg`** invece sono seri: significano che il video YouTube non esiste più, quindi il post ha un player vuoto **e** un `og:image` rotto (la thumbnail la deriva `_plugins/youtube_thumbnails.rb` da quel videoid). Per ripulirli c'è `scripts/remove_dead_youtube_embeds.rb` (`DRY_RUN=1`). Attenzione a come si verifica un video: `watch?v=` risponde **200 anche per un video inesistente**, quindi va sondato `i.ytimg.com` con uno User-Agent da browser, confrontando sempre con un id notoriamente vivo.

Due cose da sapere sui post progetto. **1)** Il sync aggiorna solo i post che portano il marker `github: <owner>/<repo>`: un post scritto a mano resta ai dati del giorno in cui fu scritto, e non viene nemmeno duplicato perche' l'URL del repo compare gia' nel corpo. Per farlo gestire dal sync basta aggiungere il marker — ma il corpo verra' rigenerato dal template, quindi conviene solo quando il corpo e' boilerplate. **2)** I controlli anti-duplicato confrontano URL e slug su **confine di parola**: con un confronto per sottostringa un repo il cui nome e' prefisso di un altro (`Docker-FFmpeg` dentro `Docker-FFmpeg-Nvenc`) veniva saltato per sempre, senza che nulla lo segnalasse.
