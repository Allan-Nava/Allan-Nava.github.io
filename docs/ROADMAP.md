...
- [ ] **Ottimizzazione immagini automatica** (#138) — *WebP fatto, AVIF no.* `scripts/optimize_images.rb` genera le varianti responsive (480/960/1440, soglia 150 KB) e le committa; `_plugins/responsive_images.rb` avvolge gli `<img>` locali in `<picture>` **solo se le varianti esistono su disco**, quindi un'immagine non convertita è servita così com'è senza rompere niente; `image-optimize.yml` (settimanale + dispatch) copre le immagini nuove. Risultato misurato: **−77% di byte immagine** sulle 27 pagine con immagini locali (18.1 → 4.0 MB), il post più pesante da 3.4 MB a 607 KB, +17.5 MB nel repo. Le 8 pagine gated restano verdi su Lighthouse. **Resta aperto l'AVIF**: il ramo `avifenc` nello script non è mai stato eseguito e il workflow non installa l'encoder — inoltre `avifenc` non ridimensiona, quindi servirebbe un resizer per avere le varianti per larghezza. Le thumbnail YouTube (`i.ytimg.com`) sono remote e restano fuori scope.
