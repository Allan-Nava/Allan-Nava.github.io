...
- **Thumbnail dei progetti: si normalizza il contenitore, non l'immagine** — le immagini dei progetti vengono dai README di altri repo e portano ognuna il proprio fondo (misurate: 6 loghi col fondo dentro il file, 4 diagrammi su bianco, 3 screenshot scuri, 1 grafico chiaro su trasparenza). La classe `.thumb-project` (in `projects.html` e nella sezione "Recent projects" di `index.html`) le mette in `contain` su una tessera `--color-thumb-bg`, con velo `--color-thumb-veil` che spegne i fondi bianchi **solo** sul tema scuro. Mai filtrare le immagini: cambiano senza preavviso. `cover` resta a blog e video (`.thumb-video`), dove la sorgente deve riempire il box — e lì valgono ancora le regole sul crop delle bande.
