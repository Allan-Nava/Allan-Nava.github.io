...
- **I `_plugins/` girano in CI ma NON sul sito pubblicato** (#144): `jekyll.yml` fa un `bundle exec jekyll build` completo — plugin inclusi — ma GitHub Pages è su `build_type: legacy` ("Deploy from a branch"), quindi a pubblicare è il builder safe-mode di GitHub, che i `_plugins/` li ignora. Risultato: niente `<picture>`/WebP/AVIF, niente lazy loading, `og:image` ancora rotto, monogrammi delle card non renderizzati. **Non c'è nessun errore in build**: Liquid in lax mode ignora i filtri sconosciuti e lascia passare il valore. Finché #144 non è risolta, qualunque cosa costruita su un plugin è verificata solo in locale — controllare sempre sul live prima di dichiararla fatta.
